...
document.getElementById('sendEmployeeReportBtn')?.addEventListener('click', async function() {
    ...
    try {
      if (reportType === 'attendance') {
        ...
        if (response.ok) {
          reportData = data;
          // IMPORTANT: These headers MUST match the keys returned by the API
          reportHeaders = ["Date", "Punch In", "Punch Out", "Status", "Punch In Location", "Punch Out Location"];
          ...
          // This block is crucial for catching API-specific errors from backend
          throw new Error(data.message || 'Failed to fetch attendance records.');
        }
      } else if (reportType === 'regularization') {
        // FIX START: Enhanced error handling for regularization report fetch
        ...
        const data = await response.json(); // Always parse JSON to get backend message
        if (response.ok) {
          reportData = data;
          // IMPORTANT: These headers MUST match the keys returned by the API's projected fields
          // Ensure these match the keys in the objects returned by the backend (e.g., 'Date', 'Original In')
      ...
      // Only attempt to send email if there's report data or it's a valid empty report.
      // The `sendReportEmail` function should be robust to handle empty `reportData` for attachment.
    ...
    if (records.length > 0) {
        records.forEach(record => {
            tableRows += `
              <tr>
                <td>${record['Date']}</td>
                <td>${record['Punch In'] || '-'}</td>
                <td>${record['Punch Out'] || '-'}</td>
                <td>${record['Status']}</td>
                <td>${record['Punch In Location'] || '-'}</td>
              </tr>
            `;
        });
    } else {
    ...
    if (records.length > 0) {
      records.forEach(record => {
        tableRows += `
          <tr>
            <td>${record['Date']}</td>
            <td>${record['Original In'] || '-'}</td>
            <td>${record['Original Out'] || '-'}</td>
            <td>${record['Modified In'] || '-'}</td>
            <td>${record['Modified Out'] || '-'}</td>
            <td>${record['Reason'] || '-'}</td>
