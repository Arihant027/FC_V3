...
def get_regularization_records_for_employee():
    ...
    try:
        ...
        formatted_records_for_export = []
        for record in records:
            formatted_records_for_export.append({
                'Date': record['date'],
                'Original In': format_time_for_display(record['original_punch_in']),
                'Original Out': format_time_for_display(record['original_punch_out']),
                'Modified In': format_time_for_display(record['modified_punch_in']),
                'Modified Out': format_time_for_display(record['modified_punch_out']),
                'Reason': record['regularized_reason'] if record.get('regularized_reason') else '-',
                'Comments': record['regularized_comments'] if record.get('regularized_comments') else '-'
            })

        return jsonify(formatted_records_for_export), 200 # Return the correctly formatted list
    except Exception as e:
...
def update_leave_request_status(request_id):
    ...
    try:
        ...
        if employee_recipients:
            ...
            {"<p><strong>Admin Comments:</strong> " + admin_comment + "</p>" if admin_comment else ""}
